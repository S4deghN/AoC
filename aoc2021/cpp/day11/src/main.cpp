...
uint64_t gain_energy(Vec<Vec<Octopus>>& table, int h, int w) {
    if (is_flashable(table, h, w) && ++table[h][w].energy > 9) {
        ...
        return 1 + std::accumulate(dirs.begin(), dirs.end(), (uint64_t)0,
                                   [&](auto acc, const auto& dir) { return acc + gain_energy(table, h + dir.first, w + dir.second); });
    } else {
        return 0;
    }
}
...
int main() {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        if (std::all_of(table.begin(), table.end(), [](const auto& row) {
                return std::all_of(row.begin(), row.end(), [](const auto& oct) { return oct.flashed; });
            })) {
            moment = i + 1;
            // break;
        }
